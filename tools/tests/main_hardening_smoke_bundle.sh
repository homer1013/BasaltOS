...
echo "[hardening] 1/7 configure invalid-board regression"
bash tools/tests/configure_invalid_board_regression.sh

echo "[hardening] 2/7 shell UX contract"
bash tools/tests/shell_ux_polish_smoke.sh

echo "[hardening] 3/7 quickstart contract"
bash tools/tests/esp32_first_success_quickstart_smoke.sh

echo "[hardening] 4/7 examples pack contract"
bash tools/tests/examples_pack_recipes_smoke.sh

echo "[hardening] 5/7 staged artifact guard"
bash tools/tests/staged_artifact_guard.sh

echo "[hardening] 6/7 board catalog drift"
bash tools/tests/board_catalog_drift_smoke.sh

echo "[hardening] 7/7 configurator API smoke"
