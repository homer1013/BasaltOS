...
def emit_basalt_config_h(
    ...
    # Module runtime settings (emitted only for enabled modules)
...
def emit_sdkconfig_defaults(
    ...
    # Module-provided sdkconfig snippets (for enabled modules only)
    mod_lines: List[str] = []
    for mid in enabled_modules:
        mod = modules.get(mid)
        if not mod:
        ...
        if isinstance(sdk_lines, list) and sdk_lines:
            mod_lines.append(f"# --- BEGIN module defaults: {mid} ---")
            mod_lines.extend([str(x) for x in sdk_lines])
            mod_lines.append(f"# --- END module defaults: {mid} ---")
            mod_lines.append("")

    # If PSRAM is not enabled via module, explicitly disable it by default.
    if "psram" not in enabled_modules:
        out_lines.append("# --- BEGIN module defaults: psram (disabled) ---")
        out_lines.append("CONFIG_SPIRAM=n")
        out_lines.append("# --- END module defaults: psram (disabled) ---")
        out_lines.append("")
    ...
    if board_profile:
        ...
        if sdk_path.exists():
            ...
            if has_psram_cfg and "psram" not in enabled_list:
                print("[configure] WARNING: existing sdkconfig enables PSRAM, but psram module is not enabled.")
