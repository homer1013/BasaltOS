...
static int analogReadSettled(uint8_t pin) {
    // Fast MCUs (e.g. RA4) can read before the resistive network settles.
    // Throw away one conversion, then take the settled sample.
...
TSPoint_kbv TouchScreen_kbv::getPoint(void) {
    ...
    for (i = 0; i < NUMSAMPLES; i++) {
        samples[i] = analogReadSettled(_yp) ADC_ADJUST;
    }
    ...
    for (i = 0; i < NUMSAMPLES; i++) {
        samples[i] = analogReadSettled(_xm) ADC_ADJUST;
    }
    ...
    int z1 = analogReadSettled(_xm) ADC_ADJUST;
    int z2 = analogReadSettled(_yp) ADC_ADJUST;
...
int TouchScreen_kbv::readTouchX(void) {
    ...
    return (1023 - (analogReadSettled(_yp)) ADC_ADJUST);
}
...
int TouchScreen_kbv::readTouchY(void) {
    ...
    return (1023 - (analogReadSettled(_xm)) ADC_ADJUST);
}
