...
static inline TickType_t ms_to_ticks(uint32_t timeout_ms) {
    ...
    // ESP-IDF interprets ticks=0 as non-blocking/timeout immediately in many APIs.
...
int hal_i2c_deinit(hal_i2c_t *i2c) {
    if (!i2c || !i2c->initialized) return -EINVAL;

    esp_err_t e = i2c_driver_delete((i2c_port_t)i2c->port);
...
int hal_i2c_set_freq(hal_i2c_t *i2c, uint32_t freq_hz) {
    ...
    // Reconfigure the peripheral (ESP-IDF classic driver does not provide a simple set-speed call)
    i2c_config_t conf = {0};
    conf.mode = I2C_MODE_MASTER;
    conf.sda_io_num = (gpio_num_t)i2c->sda_pin;
    conf.scl_io_num = (gpio_num_t)i2c->scl_pin;
    conf.sda_pullup_en = GPIO_PULLUP_ENABLE;
    conf.scl_pullup_en = GPIO_PULLUP_ENABLE;
    conf.master.clk_speed = freq_hz;

    esp_err_t e = i2c_param_config((i2c_port_t)i2c->port, &conf);
    if (e == ESP_OK) {
        i2c->freq_hz = freq_hz;
        return 0;
    }
...
int hal_i2c_probe(hal_i2c_t *i2c, uint8_t addr, uint32_t timeout_ms) {
    ...
    // Build a minimal transaction: START + address(write) + STOP.
    ...
    esp_err_t ex = i2c_master_cmd_begin((i2c_port_t)i2c->port, cmd, ms_to_ticks(timeout_ms));
    i2c_cmd_link_delete(cmd);

    if (ex == ESP_OK) return 0;

    // NACK on address commonly maps to ESP_FAIL (implementation-dependent); treat as ENODEV.
    // If it's a timeout or invalid state, map those precisely.
